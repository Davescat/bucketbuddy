    ...
    return (
      <div className='bucket-path'>
        <Breadcrumb >Current Directory:&nbsp;
          ...
          {this.props.pathInfo.depth !== 0 && <BreadcrumbSection onClick={this.changePath} depth={0} active={this.props.pathInfo.depth === 0} >{process.env.REACT_APP_AWS_BUCKET}</BreadcrumbSection>}
          {arr.map((file, index) => {
            if (file === '/') {
              ...
              return index !== (this.props.pathInfo.depth * 2) - 1 ?
                <BreadcrumbSection onClick={this.changePath} depth={sectionDepth}>{file}</BreadcrumbSection> :
