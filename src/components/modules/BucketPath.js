        ...
        return (
            <div className='bucket-path'>
                <Breadcrumb>Current Directory:
                    {this.props.pathInfo.depth === 0 && <BreadcrumbDivider>/</BreadcrumbDivider>}
                    {this.props.pathInfo.depth > 0 && arr.map((file, index) => {
                        if (file === '/') {
                            return <BreadcrumbDivider>/</BreadcrumbDivider>
                        } else {
                            return index !== (this.props.pathInfo.depth * 2) - 1 ? <BreadcrumbSection >{file}</BreadcrumbSection> : <BreadcrumbSection active>{file}</BreadcrumbSection>
                        }
                    })}
                </Breadcrumb>
            </div>
